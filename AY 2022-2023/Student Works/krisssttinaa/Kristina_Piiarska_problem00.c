...
do {
  ...
  printf("Enter the type of animal (D for dog, C for cat, B for bird, R for reptile): ");
  scanf("%c", &cAnimal);
  ...
  if (cAnimal == 'D' || cAnimal == 'd') {
    ...
    if (cNeutered == 'Y' || cNeutered == 'y') {
      ...
      printf("\n Insurance fee for a dog that has not been neutered costs $60.0");
    }
  } else if (cAnimal == 'B' || cAnimal == 'b' || cAnimal == 'R' || cAnimal == 'r') {
    printf("\n Insurance fee for a cat that has been neutered costs $0.00");
  } else {
